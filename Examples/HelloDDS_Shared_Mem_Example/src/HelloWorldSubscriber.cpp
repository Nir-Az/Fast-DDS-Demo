        ...
        DataReader* reader)
{
    ...
    if (reader->take_next_sample(hello_.get(), &info) == ReturnCode_t::RETCODE_OK)
    {
	    ...
	    auto time_now_ms =  duration_cast< milliseconds >( system_clock::now().time_since_epoch()).count();
            std::cout << "Message " << hello_->message() << " " << hello_->index()
                      << " RECEIVED With " << data_size << "(bytes) of Data. DataEnd = "
                      << (char*)&hello_->data()[data_size - 9] 
			<< " Time Since Published: " << (time_now_ms - hello_->publish_time()) << " [ms]"
			<< std::endl;
        }
    }
}
